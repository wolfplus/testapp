...
@Component({
  selector: 'app-match-card-tablet',
  templateUrl: './match-card-tablet.component.html',
  styleUrls: ['./match-card-tablet.component.scss']
})
export class MatchCardTabletComponent implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.match.duration === undefined) {
      this.match.duration = getDurationInMinutes(this.match.startAt, this.match.endAt);
    }
